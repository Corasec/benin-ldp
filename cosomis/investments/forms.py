...
class InvestmentsForm(forms.Form):
    ...
    def clean_investments(self):
        ...
        package = Package.objects.get_active_cart(user=self.context["user"])
...
class PackageApprovalForm(forms.Form):
    ...
    reject_reason = forms.CharField(
        ...
        help_text=_(
            "You are about to send this submission for "
    ...
    def save(self):
        ...
        if "reject_reason" in self.cleaned_data and self.cleaned_data[
            "reject_reason"
        ] not in [None, ""]:
            package.rejection_reason = self.cleaned_data["reject_reason"]
            package.status = Package.REJECTED
            if (
                "no_resubmission" in self.cleaned_data
                and self.cleaned_data["no_resubmission"]
            ):
