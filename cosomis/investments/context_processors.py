...
def notifications(request):
    resp = {"pending_approvals": 0, "has_urgent_approvals": False}
    try:
        if request.user.is_moderator or request.user.is_superuser:
            max_response_day = (
                settings.MAX_RESPONSE_DAYS
                if hasattr(settings, "MAX_RESPONSE_DAYS")
                else 3
            )
            ...
            has_urgent_packages = packages_qs.filter(
                updated_date__lte=urgent_day
            ).exists()
...
def cart_items(request):
    try:
        ...
        return {"cart_items": package.funded_investments.all().count()}
    except TypeError:
        return {"cart_items": 0}
