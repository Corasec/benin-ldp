...
def parse_fields(fields, field_types, response):
    ...
    for key, field_info in fields.items():
        if "fields" in field_info:  # Nested fields
            ...
            nested_field_types = (
                field_types[key]["properties"]
                if key in field_types and "properties" in field_types[key]
                else {}
            )
            nested_response = response[key] if key in response else {}
            nested_responses = parse_fields(
                nested_fields, nested_field_types, nested_response
            )
