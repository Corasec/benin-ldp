...
def update_or_create_priorities_document(priorities_document):
    ...
    if "form_response" in priorities_document:
        try:
            if (
                priorities_document.get("form_response")
                and "agriculteursEtEleveurs" in priorities_document["form_response"][0]
            ):
                for idx, priority in enumerate(
                    priorities_document["form_response"][0]["agriculteursEtEleveurs"][
        ...
        try:
            if (
                priorities_document.get("form_response")
                and "groupeDesFemmes" in priorities_document["form_response"][1]
            ):
                for idx, priority in enumerate(
                    priorities_document["form_response"][1]["groupeDesFemmes"][
                        "besoinsPrioritairesDuGroupe"
                    ]
                ):
                    investment = Investment.objects.filter(
        ...
        try:
            if (
                priorities_document.get("form_response")
                and "groupeDesJeunes" in priorities_document["form_response"][2]
            ):
                for idx, priority in enumerate(
                    priorities_document["form_response"][2]["groupeDesJeunes"][
                        "besoinsPrioritairesDuGroupe"
                    ]
                ):
                    investment = Investment.objects.filter(
        ...
        try:
            if (
                priorities_document.get("form_response")
                and "groupeEthniqueMinoritaires"
                in priorities_document["form_response"][3]
            ):
                for idx, priority in enumerate(
                    priorities_document["form_response"][3][
