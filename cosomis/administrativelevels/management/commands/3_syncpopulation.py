...
def update_or_create_adm_document(client, population_document):
    ...
    selector = {"adm_id": adm_id, "type": "administrative_level"}
    ...
    if "form_response" in population_document:
        if population_document.get("form_response"):
            extracted_population_data = extract_population_data(
                population_document["form_response"]
            )
    ...
    if adm_object:
        ...
        adm_object.population_handicap = extracted_population_data[
            "population_handicap"
        ]
        adm_object.population_agriculture = extracted_population_data[
            "population_agriculture"
        ]
        adm_object.population_breeders = extracted_population_data[
            "population_breeders"
        ]
        adm_object.population_minorities = extracted_population_data[
            "population_minorities"
        ]
        adm_object.save()
...
def extract_population_data(form_response):
    ...
    for entry in form_response:
        if "population" in entry:
            extracted_population_data["total_population"] = entry["population"].get(
                "populationTotaleDuVillage", 0
            )
            extracted_population_data["population_men"] = entry["population"].get(
                "populationNombreDeHommes", 0
            )
            extracted_population_data["population_women"] = entry["population"].get(
                "populationNombreDeFemmes", 0
            )

        elif (
            "personnesVulnerables" in entry
            and entry["personnesVulnerables"] is not None
        ):
            persons_vulnerable = entry["personnesVulnerables"]

            if (
                "populationEnfants" in persons_vulnerable
                and persons_vulnerable["populationEnfants"] is not None
            ):
                extracted_population_data["population_young"] = persons_vulnerable[
                    "populationEnfants"
                ].get("enfantsTotal", 0)

            if (
                "populationPersonnesAgees" in persons_vulnerable
                and persons_vulnerable["populationPersonnesAgees"] is not None
            ):
                extracted_population_data["population_elder"] = persons_vulnerable[
                    "populationPersonnesAgees"
                ].get("populationPersonnesAgeesTotal", 0)

            if (
                "populationPersonnesHandicape" in persons_vulnerable
                and persons_vulnerable["populationPersonnesHandicape"] is not None
            ):
                extracted_population_data["population_handicap"] = persons_vulnerable[
                    "populationPersonnesHandicape"
                ].get("populationPersonnesHandicapeTotal", 0)
