...
def update_or_create_document(document):
    ...
    if document_type == "phase":
        existing_phase = Phase.objects.filter(
            no_sql_db_id=object_id, village=int(administrative_level_id)
        )
            ...
            try:
                Phase.objects.create(
                    ...
                    village=AdministrativeLevel.objects.get(
                        no_sql_db_id=administrative_level_id
                    ),
                    name=document["name"],
                ...
                print(
                    ...
                    document["name"],
                    document["administrative_level_id"],
                )
        else:
            try:
        ...
        existing_activity = Activity.objects.filter(
            no_sql_db_id=object_id, phase__village=int(administrative_level_id)
        )
            ...
            try:
                ...
                Activity.objects.create(
                    ...
                    phase=Phase.objects.get(
                        no_sql_db_id=document["phase_id"], village=administrative_level
                    ),
                    name=document["name"],
                    description=document["description"],
                    order=document["order"],
            ...
            try:
                ...
                Task.objects.create(
                    ...
                    activity=Activity.objects.get(
                        no_sql_db_id=document["activity_id"],
                        phase__village=administrative_level,
                    ),
                    name=document["name"],
                    description=document["description"],
                    order=document["order"],
