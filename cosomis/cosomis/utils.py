...
def link_infrastures_to_subproject():
    ...
    for subproject in subprojects:
        for _subproject in subprojects:
            if (
                subproject.id != _subproject.id
                and subproject.number < _subproject.number
                and subproject.joint_subproject_number
                == _subproject.joint_subproject_number
            ):
        ...
        if not subproject.check_step(s):
            ...
            if subproject.approval_date_cora and s.ranking == 1:  # 1
                subproject_step.begin = (
                    subproject.approval_date_cora - datetime.timedelta(days=14)
                )
                ...
                if subproject_step_current:
                    subproject_step.begin = (
                        subproject_step_current.begin + datetime.timedelta(days=7)
                    )
                    ...
                    subproject_step.begin = (
                        subproject.approval_date_cora + datetime.timedelta(days=3)
                    )
                else:
    ...
    for subproject in subprojects:
        if subproject.current_level_of_physical_realization_of_the_work:
            ...
            if current_level == "Remise du site".title():  # Remise du site
                ...
                if (
                    subproject_step_progress.step.has_levels
                    and not subproject_step_progress.check_step(
                        subproject.current_level_of_physical_realization_of_the_work
                    )
                ):
                    subproject_level = Level()
                    subproject_level.wording = (
                        subproject.current_level_of_physical_realization_of_the_work
                    )
...
def all_functions_call():
    attribute_component_to_subprojects(
        Subproject.objects.all(), Component.objects.get(id=2)
    )
