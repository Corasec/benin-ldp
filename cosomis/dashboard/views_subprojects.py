        ...
        for i in range(len(_sectors)):
            ...
            for column in columns_listes:
                number = all_subprojects.filter(
                    ...
                    subproject_type_designation__in=(
                ...
                datas[column[1]][
                    count
                ] = number  # str(number).rjust(characters_length,'0')
                number_toal += number
...
class DashboardSubprojectsStepsAlreadyTrackListView(
    ...
    def summary_subprojects_by_steps_already_track(
        ...
        for step in Step.objects.all():
            ...
            datas[_("Number")][count] = all_subprojects.filter_by_steps_already_track(
                Subproject, step_id=step.id
            ).count()
...
class DashboardSubprojectsCurrentStepsListView(
    ...
    def summary_subprojects_by_current_steps(self, all_subprojects, characters_length):
        ...
        for step in Step.objects.all():
            ...
            datas[_("Number")][count] = all_subprojects.filter_by_step(
                Subproject, step_id=step.id
            ).count()
